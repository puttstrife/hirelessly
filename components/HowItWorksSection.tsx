...
function DeployPlaceholder() {
  ...
  return (
    <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 20, padding: 28, width: "100%" }}>
      <div style={{ fontSize: "0.6875rem", color: "var(--ink-faint)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 16 }}>Connected Systems</div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        {integrations.map((name, i) => (
          <div key={name} style={{ background: "var(--surface-2)", borderRadius: 10, padding: "10px 14px", display: "flex", alignItems: "center", gap: 8, animation: `fadeUp 0.4s ease ${i * 0.06}s both` }}>
            <span style={{ width: 8, height: 8, borderRadius: "50%", background: "var(--signal)", display: "inline-block", flexShrink: 0 }} />
            <span style={{ fontSize: "0.8125rem", color: "var(--ink)", fontWeight: 500 }}>{name}</span>
          </div>
        ))}
      </div>
      <div style={{ marginTop: 16, background: "rgba(15,143,104,0.08)", border: "1px solid rgba(15,143,104,0.25)", borderRadius: 10, padding: "10px 14px", fontSize: "0.8125rem", color: "var(--signal)", fontWeight: 600 }}>
        Ready to connect to your existing stack
      </div>
    </div>
  );
}
    ...
    <div style={{ display: "grid", gap: 48 }} className="how-grid">
      ...
      <div>
        ...
        <div style={{ position: "relative" }}>
          <div style={{ position: "absolute", left: 19, top: 32, bottom: 32, width: 2, background: "var(--horizon)" }} />
          {STEPS.map((s, i) => {
            ...
            return (
              ...
              >
                <div style={{
                  ...
                  boxShadow: isActive ? "0 0 0 4px rgba(43,71,214,0.16)" : "0 0 0 4px var(--surface-2)",
                ...
                <div style={{ paddingTop: 8 }}>
                  ...
                  <div style={{ fontSize: "0.875rem", color: "var(--text-muted)", lineHeight: 1.6, display: isActive ? "block" : "none" }}>{s.desc}</div>
                </div>
              </button>
            );
          })}
